...
export class PreventionDashboardComponent implements OnInit, AfterViewInit{
  ...
  createBar(subPlans) {
    ...
    for (let i = 0; i < subPlans.length; i++) {
      ...
      this.prevConcepts.datasets[0].metadata.push(subPlans[i]);
      if(subPlans[i].subPlans.length > 1){
        this.prevConcepts.datasets[0].backgroundColor.push('#315a2d');
      } else {
        this.prevConcepts.datasets[0].backgroundColor.push('#87cf78');
      }
    }
  ...
  callback(data){
    ...
    if(this.mainPlan.score == -1) {
      localStorage.setItem('Followup and Prevention', 'no dada');
      this.pageError = true;
    } else {
